window.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        document.querySelector(
            ".card--total-expenses .card__value"
        ).textContent = `${Number(balance.total_group_spending).toFixed(2)} zł`;
        document.querySelector(
            ".card--your-debt .card__value"
        ).textContent = `${Number(balance.you_owe_now).toFixed(2)} zł`;
        document.querySelector(
            ".card--others-owe .card__value"
        ).textContent = `${Number(balance.others_owe_you).toFixed(2)} zł`;
        ...
        groupDetails.members.forEach((member) => {
            ...
            const name =
                member.id === userId ? `${member.name} (Ja)` : member.name;
...
document
    .querySelector("#create-expense-modal .button")
        ...
        try {
            ...
            const payload = {
                ...
                splitValues: (() => {
                    if (currentSplitType === "equal") {
                        return Array.from(
                            document.querySelectorAll(
                                "#split-equal-list .split-eq-cb:checked"
                            )
                        ).map((cb) => cb.dataset.id);
                    }
                    if (currentSplitType === "exact") {
                        return Array.from(
                            document.querySelectorAll("#split-exact-list input")
                        ).map((i) => ({
                            userId: i.dataset.id,
                    ...
                    return Array.from(
                        document.querySelectorAll("#split-percent-list input")
                    ).map((i) => ({
                        userId: i.dataset.id,
                        percent: parseFloat(i.value) || 0,
                    }));
                })(),
            };
...
document
    .getElementById("settle-expenses-btn")
    ?.addEventListener("click", async () => {
        const overlay = document.getElementById("modal-overlay");
        ...
        try {
            const res = await fetch(`/group/${groupId}/settlements`, {
                credentials: "include",
            });
        ...
        <input type="checkbox" class="settle__checkbox" data-from-id="${
            s.from.id
        }" data-to-id="${s.to.id}" data-amount="${s.amount}">
      `;
                list.appendChild(li);
            });
        } catch (e) {
...
document
    .getElementById("settle-confirm-btn")
    ?.addEventListener("click", async () => {
        const overlay = document.getElementById("modal-overlay");
        const modal = document.getElementById("settle-expenses-modal");
        const urlParams = new URLSearchParams(window.location.search);
...
payerSelector.addEventListener("click", async () => {
    ...
    if (groupMembers.length === 0) {
        ...
        try {
            ...
            const res = await fetch(`/group/${groupId}/details`, {
                credentials: "include",
            });
...
payerConfirmBtn.addEventListener("click", () => {
    ...
    payerSelector.textContent = groupMembers.find(
        (m) => m.id === expensePayerId
    ).name;
...
function renderSplitLists(members) {
    equalList.innerHTML = "";
    members.forEach((m) => {
        equalList.insertAdjacentHTML(
            "beforeend",
            `
      <li>
        <img src="assets/person_icon.svg" class="members__icon" alt="">
        <span class="members__name">${m.name}${
                m.id === currentUserId ? " (Ja)" : ""
            }</span>
    ...
    members.forEach((m) => {
        percentList.insertAdjacentHTML(
            "beforeend",
            `
      <li>
        <img src="assets/person_icon.svg" class="members__icon" alt="">
...
tabs.forEach((tab) =>
    tab.addEventListener("click", () => {
        ...
        panes.forEach((p) =>
            p.classList.toggle("hidden", p.dataset.content !== currentSplitType)
        );
    })
);

exactList.addEventListener("input", () => {
    const sum = Array.from(exactList.querySelectorAll("input")).reduce(
        (s, i) => s + parseFloat(i.value || 0),
        0
    );
    splitModal.querySelector(
        '[data-content="exact"] .split-summary'
    ).textContent = `${sum.toFixed(2)} zł z ${totalAmount.toFixed(2)} zł`;
});
percentList.addEventListener("input", () => {
    const sum = Array.from(percentList.querySelectorAll("input")).reduce(
        (s, i) => s + parseFloat(i.value || 0),
        0
    );
    splitModal.querySelector(
        '[data-content="percent"] .split-summary'
